...
impl Payload {
    pub fn new(address: String, method: String) -> Self {
        Self {
            address,
            method,
            block_hash: "0xd4d4069f6842e848e3cd37f2a7b70365edb1f2ea67d6cf9d10ce8fea9d56cea8"
                .to_string(),
            block_number: "0x00000000".to_string(),
            era: "0x0000".to_string(),
            genesis_hash: "0xd4d4069f6842e848e3cd37f2a7b70365edb1f2ea67d6cf9d10ce8fea9d56cea8"
                .to_string(),
...
pub async fn enable() -> Result<Vec<Extension>, String> {
    let list = web3_enable_extensions("Polymesh Yew App")
        .await
        .map_err(fmt_err)?;
    let extensions: Vec<Extension> = from_value(list).map_err(|e| e.to_string())?;
    Ok(extensions)
}

pub async fn accounts() -> Result<Vec<Account>, String> {
    let list = web3_get_accounts().await.map_err(fmt_err)?;
    let accounts: Vec<Account> = from_value(list).map_err(|e| e.to_string())?;
    Ok(accounts)
}

pub async fn sign_payload(payload: Payload) -> Result<JsValue, String> {
    let payload = serde_wasm_bindgen::to_value(&payload).map_err(|e| e.to_string())?;
    let sig = web3_sign_payload(payload).await.map_err(fmt_err)?;
    Ok(sig)
}
